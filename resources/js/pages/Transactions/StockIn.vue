        ...
        <form @submit.prevent="submit" class="space-y-6 mt-6">
            ...
            <div class="p-2 bg-white border border-slate-300 rounded-xl overflow-hidden">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-100 border-b border-slate-300 text-[10px] font-bold uppercase tracking-widest text-slate-500">
                            <th class="px-6 py-3">Property Description</th>
                            <th class="px-6 py-3 w-32 border-l border-slate-300 text-center">Qty</th>
                            <th class="px-6 py-3 w-40 border-l border-slate-300 text-right">Unit Cost</th>
                    ...
                    <tbody class="divide-y divide-slate-100 font-medium">
                        <tr v-for="(line, index) in form.line_items" :key="index">
                            ...
                            <td class="px-4 py-3 border-l border-slate-50">
                                <input v-model="line.unit_cost" type="number" step="0.01" class="w-full border border-slate-300 text-left h-9 rounded-sm focus:ring-slate-900 focus:border-slate-900" :disabled="recentlySubmitted" />
                            </td>
                ...
                <div v-if="!recentlySubmitted" class="p-3 bg-slate-100 border-t border-slate-100 flex justify-center mt-2 rounded-b-lg">
                    <button @click="addItemRow" type="button" class="text-[10px] font-bold text-slate-900 uppercase tracking-[0.2em] flex items-center gap-2 hover:text-indigo-600 transition-colors">
            ...
            <div class="flex justify-end pt-4">
                <div v-if="recentlySubmitted" class="flex gap-3">
                    <button @click="triggerExport" type="button" class="flex items-center px-8 py-4 bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] font-bold rounded-sm shadow-sm transition-colors uppercase tracking-[0.2em]">
